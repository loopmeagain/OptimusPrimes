...
main = do  print  (sumarTernas (obtenerTernas [1..1000]))
           return (sumarTernas (obtenerTernas [1..1000]))

esPrimo :: Integer -> Bool
esPrimo 1 = False
esPrimo 2 = True
esPrimo n = not((length [x | x <- [2 .. n-1], mod n x == 0]) > 0 )

memoizedEsPrimo :: Integer->Bool
...
sumar (x,y,z)					= x+y+z
sumarTernas 					= map sumar 


obtenerPrimosDeTerna (x,y,z)	= (x+y,y+z)
getPrimosFromTernas 			= (map obtenerPrimosDeTerna ).obtenerTernas 

ternaEsMultiploDe3:: (Integer,Integer,Integer)->Bool

ternaEsMultiploDe3 				= (==0).(flip mod 3).sumar

sonMultiplosDe3:: [(Integer,Integer,Integer)]->Bool

sonMultiplosDe3 				= all ternaEsMultiploDe3
